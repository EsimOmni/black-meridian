extends SceneTree
## Throwaway gate — validate the transit-spine hero against the 'building' spec.

const GLBValidator := preload("res://tools/validation/glb_validator.gd")

func _init() -> void:
	var spec := GLBValidator.spec_for(&"building")
	var report := GLBValidator.validate_file("res://assets/city/glasswharf_dock/transit_spine.glb", spec)
	print("=== transit_spine vs 'building' spec ===")
