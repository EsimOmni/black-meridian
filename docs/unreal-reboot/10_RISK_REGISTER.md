...
| **Detection** | `[V]` **S5, as built:** `Test_Save_AdditiveField` + `Test_Save_FixtureStillLoads` against the committed `Tests/Fixtures/Saves/v2.bmsav`, which is **never regenerated** — regenerating it is what turns the guard into theatre. Plus `BM.Save.AppendedFieldLoadsAsDefault` (the mechanism) and `BM.Save.TruncatedFileIsStillRefused` (the line it must not cross), and the version-bump checklist ([05](05_DATA_MODEL.md) §7). **A binary archive's field ORDER is the format**, so an insertion is a silent break the fixture is the only thing that sees — proved by mutation in S4 and **re-proved in S5** (swapping two appended fields turned exactly one test red) |
| **Mitigation** | Additive-only within a version; **append-only enums AND append-only struct fields** — `[V]` S4 proved the second half by mutation: swapping two same-type fields inside `Serialize()` left 55 of 56 tests green and was caught by the committed fixture alone. Save/load built early (S4) so every later slice is tested against it; jobs store runtime state only. ⚠️ **S5 WAS the first test of this, and the contract FAILED IT** — see the row below |
| **`[V]` S5 correction** | **The additive contract was documented in S4 and never implemented.** Measured: appending the seven rival fields made `v1.bmsav` fail to load outright (`result=Corrupt`). The rule was right; the MECHANISM was missing. The oracle appended fields eleven times (P05b–P16) without ever bumping `SAVE_VERSION` because its format is **self-describing** (`var_to_str` — every field carries its name, so `d.get(key, default)` works). Ours is **positional**: order, no names. **A rule cannot be ported without the mechanism that makes it true.** S5 added `FRecordScope` — a per-record byte-length prefix, the binary equivalent of `d.get(key, default)`. `CurrentVersion` went to **2** for the CONTAINER RESHAPE, *not* for the appended fields; `v1.bmsav` was deleted (reading it would need a second read mode = a migration path, which [07](07_IMPLEMENTATION_ROADMAP.md) §S4 prohibits) and `v2.bmsav` is the baseline. **From S5 onward an appended field genuinely does keep old saves loading.** Evidence: `Docs/gates/S5.md` Finding 6 |
| **`[V]` The rule, for S6/S7/S8** | Adding a field does **NOT** bump — the old save keeps loading and new fields take their defaults. Bump **only** when a field is removed, reordered or changes meaning, and at that point refusing the old save is what you actually want (the check is `!=`, so a bump refuses past *and* future saves alike). ⚠️ The fixture catches a **reorder**; it does **not** catch a field you forget entirely — that is what the every-field-non-default discipline in `FixtureCampaign()` is for |
| **Owner** | Architect |
