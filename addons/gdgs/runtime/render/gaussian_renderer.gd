...
func _rasterize_state(state, point_count: int) -> void:
	...
	for radix_shift_pass in range(4):
		# Godot 4.7 compat: each sort stage declares a different push-constant block
		# size (upsweep 8B, spine 4B, downsweep 12B) and 4.7 validates exact sizes.
		...
		state.pipelines["radix_sort_upsweep"].call(state.context, compute_list, RenderingDeviceContext.create_push_constant([radix_shift_pass, in_offset]), [], state.descriptors["grid_dimensions"].rid, 0)
		state.pipelines["radix_sort_spine"].call(state.context, compute_list, RenderingDeviceContext.create_push_constant([radix_shift_pass]))
		state.pipelines["radix_sort_downsweep"].call(state.context, compute_list, RenderingDeviceContext.create_push_constant([radix_shift_pass, in_offset, out_offset]), [], state.descriptors["grid_dimensions"].rid, 0)
	state.context.compute_list_end()
