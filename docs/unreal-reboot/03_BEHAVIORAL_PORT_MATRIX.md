...
| **Erode vs remove** | Job resolution *erodes the strongest* case passively; EVIDENCE_CHAIN jobs *remove a targeted* case | `[V]` `erode_strongest` vs `JobDirector` `remove_case` | **PRESERVE** — two distinct mechanisms | `FBMEvidence` | `[V]` **S3 `BM.Jobs.PostOutcomeOriginBranches`** — asserts the TARGETED case is removed while the strongest survives. The sketched `Test_Evidence_ErodeVsRemove` was never written under that name; the behavior is covered |
...
| **Expiry outcome** | Fixed `{evidence 0.3, public_fear 0.1, delayed_consequence 0.5}` | `[V]` `expired()` | **PRESERVE** — ⚠️ the triple is load-bearing beyond its values: `delayed_consequence = 0.5` clears the 0.25 follow-up threshold, so **an expired job ALWAYS spawns its successor.** That is the mechanism by which the loop never empties (S3 correction) | `FBMJobResolution` | `GV-JOB-02` |
| **Concurrency cap** | `MAX_CONCURRENT_JOBS = 3`; a trigger at cap is **dropped**, not queued | `[V]` `_try_offer` | **PRESERVE** (with the narrative exception below) — ⚠️ the **dedupe-before-cap ORDER is NOT a behavior**: both guards are side-effect-free predicates returning `false`, so swapping them is observationally identical (proved exhaustively; in Godot it decides only which `print()` fires). Keep the oracle's order because it is free to keep, and **do not add an assertion chasing it** (S3 correction) | `FBMJobDirector` | `Test_Job_CadenceCap` |
...
| **Follow-up scheduling** | `delayed_consequence ≥ 0.25` schedules a follow-up 20 ticks later | `[V]` `FOLLOWUP_THRESHOLD/LEAD_TICKS` | **PRESERVE** — ⚠️ but a job expiring *inside* `AdvanceTick` leaves its follow-up at **`LEAD − 1`, not `LEAD`**: deadlines run first, then the follow-up pass decrements **every** entry including the one just appended. Measured `ticks_left = 19`. An assertion written from "keeps the full lead" **passed the mutant and would have failed the correct port** (S3 correction) | `FBMJobDirector` | `GV-JOB-03` |
...
| **Rebuild-from-save** | `JobTemplates.by_id` → else `JobGenerator.rebuild(id)` → else drop with warning; burycase returns null if the case is gone | `[V]` `save_service.gd` load path | **PRESERVE** incl. the honest-null — ⚠️ and **the drop still returns TRUE**: the load SUCCEEDS with that job absent, so the restored campaign legitimately differs from the snapshot (measured, 3 saved / 1 restored). Also `[V]` the two halves have **disjoint domains** (`Rebuild` needs the `gen@` prefix, `ById` matches ids without it), so their ORDER cannot be pinned by any test — S4 | `UBMSaveSubsystem` | `Test_Save_JobRebuildByteIdentical` |
...
| **Float-exact round-trip** | `var_to_str` chosen over JSON *specifically* to preserve replay determinism | `[V]` `save_codec.gd` doc + `tasks/lessons.md` | **ADAPT** — UE `FArchive` binary is exact natively; the *requirement* persists. ⚠️ And `var_to_str` **sorts dictionary keys**, so the oracle's own round-trip test could not see a reordered encode — in a binary archive **field order IS the format** (`04` §8.1). The port's gate is *stricter* and needed a committed-binary fixture the oracle never had | `FBMSaveCodec` | `BM.Save.RoundTripIsExact` + `FixtureStillLoads` |
| **Hard version refusal** | Mismatch → refuse whole load, leave state untouched, no migration | `[V]` `decode_state` + `load_game`; verified by the round-trip runner | **PRESERVE** — ⚠️ but the check is `int(version) != SAVE_VERSION`, a **coercion**: a *string* `"1"` and a *float* `1.0` both LOAD, `"abc"` refuses. Unreproducible in binary → recorded bounded divergence (S4). The `!=` **is** load-bearing: a *future* version refuses too, and `<` would be a migration path by accident | `UBMSaveSubsystem` | `BM.Save.VersionMismatchIsRefused` |
| **Additive tolerance** | New fields added within a version via `.get(key, default)` | `[V]` throughout `decode_*` | **ADAPT** — `[V]` **one explicit version int + default member initializers.** Corrected in S4: ~~`FArchive` custom version + defaulted UPROPERTYs~~ — `FCustomVersion` is for *reflected* serialization and there is none, and there are no `UPROPERTY`s at all. ⛔ New fields must be **APPENDED**, never inserted (`04` §8.1) | `FBMSaveCodec` | `BM.Save.AdditiveFieldDefaults` |
| **Load leaves game paused** | Verified assertion in the smoke runner | `[V]` `bootstrap_smoke_runner` "ok: load leaves the game paused" | **PRESERVE** — ⚠️ **shipped with NO automated coverage, and that was measured** (S4): moving the pause after the restore leaves the suite green. Reaching `UBMTimeSubsystem` needs a live `GameInstance`, which trips the S2 `ClassWithin` ensure. Protected by review + a header comment; closure is `08` §7's `FT_Save_*` | `UBMSaveSubsystem` | ⛔ **none yet** — see `08` §8 item 6 |
