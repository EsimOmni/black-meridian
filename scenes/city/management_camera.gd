...
func _process(delta: float) -> void:
	...
	if move != Vector3.ZERO:
		...
		_pan_origin += move
		_pan_origin.x = clampf(_pan_origin.x, -40.0, 40.0)
		_pan_origin.z = clampf(_pan_origin.z, -40.0, 40.0)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		_pan_origin.x = clampf(_pan_origin.x - event.relative.x * 0.04, -40.0, 40.0)
		_pan_origin.z = clampf(_pan_origin.z - event.relative.y * 0.04, -40.0, 40.0)
